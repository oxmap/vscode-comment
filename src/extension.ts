...
export function activate(ctx:vscode.ExtensionContext) {
	...
	vscode.commands.registerCommand('extension.tsdoc-comments', () => {
		executeComment('typescript');
	});

	vscode.commands.registerCommand('extension.jsdoc-comments', () => {
		executeComment('javascript');
	});
}

function executeComment(curLang: string) {
	...
	if (lang == curLang) {
		...
		var outputMessage: string = `Please select a ${curLang} function signature`;
		...
		if (params.length > 0) {
			var textToInsert = functionParser.getParameterText(params, returnText, curLang);
			vscode.window.activeTextEditor.edit((editBuilder: vscode.TextEditorEdit) => {
				...
				var tabSize: string | number = vscode.window.activeTextEditor.options.tabSize;
