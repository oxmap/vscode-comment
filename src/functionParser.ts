...
export function getParameterText(paramList: paramDeclaration[], returnText: string, curLang: string): string {
	var textToInsert: string = '';
	textToInsert = textToInsert + '/**\n *';
	paramList.forEach(element => {
		if (element.paramName != '') {
			textToInsert = textToInsert + ' @param ';
			if (curLang === 'javascript') {
...
export function getParameters(text: string): paramDeclaration[] {
	...
	var index = 0;
	//Remove modifiers and spaces
	text = text.replace(/((public|protected|private)\s+)|readonly\s+/g, '');
	text = text.replace(/\s+/g, '');
